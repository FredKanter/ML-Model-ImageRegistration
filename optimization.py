...
import time
import copy
#from outdated.reduce_BFGS import reduce_BFGS
from learn_optimization.armijo_BFGS import limited_bfgs
import utils.batch_calculation as bc
import image_registration.reg_utils.tools as tools
# TODO check if scale_t0_gridres is redundant with scalegrid
from image_registration.reg_utils.grids import Grid, prolong_grid, scale_to_gridres
...
class L_BFGS(Solver):
    ...
    def solve(self, fng, x0, learn_param, timed=False, **kwargs):
        ...
        if self.LS:
            ...
            xk, fks, gks = reduce_BFGS(fng, x0, Q, timed=timed,
                                       **{'lr': self.lr, 'maxIter': self.iter, 'maxBFGS': self.maxBFGS, **kwargs})
    ...
    def _prepare_hessian(self, x0, learn_param):
        ...
        if len(sz) == 1:
            ...
            # needed for learning H0, but slows down pure BFGS, discarded for learning with MetaNet
            # less memory use (sparse tensor for Q?)
            if not self.sparse:
            ...
            return insert_param
        #     n = x0.size(1)
        #     H0 = torch.zeros((n, n), dtype=x0.dtype, device=learn_param.device)
        #     ids = torch.triu_indices(n, n)
        #     H0[ids[0], ids[1]] = learn_param
...
class ML_wrapper(Solver):
    ...
    def update_ml(self, w, wk, fcn, level_grid, m_ML, level, **params_fct):
        ...
        # define new H0 for each level (d2S calculated for each level in FAIR)
